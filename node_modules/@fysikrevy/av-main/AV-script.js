...
function serverSavedNEJ(){
   return fetch( "./nej.nej" )
      .then( res => res.text() )
      .then( text => text.split( "\n" )
                         .map( line => line.split( " " ) )
                         .filter( line => line.length == 3 )
                         .forEach( line => slideBuildList( +line[0] )[ +line[1] - 1 ]
                                             .classList
                                             .toggle( "nej" )
                         )
      );
...
function setNEJ(silent) {
  ...
  if (silent !== true) {
    ...
    xhr.setRequestHeader('slide',curSlide.toString());
    xhr.setRequestHeader('item',$( slideBuildList( curSlide ) ).filter( ":not( .to-build )" )
                                                               .length.toString()
    );
    xhr.send();
    ignNej++;
  ...
  for (var i = 0; i < media.length; i++) {
    ...
    if (media[i].classList.contains('last-build')
	 ...
	 && media[i].parentNode.classList.contains('last-build'))
      ...
      media[i].play();
      if ( overSide ) media[i].muted = true;
    }
  }
};
